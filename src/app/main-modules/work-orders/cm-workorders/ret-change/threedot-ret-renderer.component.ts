...
@Component({
    selector: 'threeDot-ret-button-renderer',
    template: `<button mat-icon-button [matMenuTriggerFor]="reportbuilderEditorMenu" aria-label="Example icon-button with a menu">
        ...
        <mat-menu #reportbuilderEditorMenu="matMenu" class="reportbuilder-editor-menu-render" xPosition="before">
            <button mat-menu-item (click)="woClick($event)">
                <span>Workorder Details</span>
            </button>
            <button mat-menu-item (click)="cellDetailsClick($event)">
...
export class ThreeDotRETRenderer implements ICellRendererAngularComp {
    params;
    enabled: Boolean;
    dataTest: any = false;
