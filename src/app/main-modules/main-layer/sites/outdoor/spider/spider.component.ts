...
@Component({
    ...
    <div class="spidermain-wrapper">
        <div class="spideroverlay">
            <img src="assets/data/layers/nominalssites/0.svg">
...
export class SpiderComponent implements OnInit {
    ...
    drawSVG(element, svgGroup, data, ref?) {
        ...
        for (var i = 0, count = bands.length; i < count; i++) {
            ...
            var cellGroupImageFor = cellGroup.append('foreignObject')
                .attr({
                    'width': 45,
                    'height': 45.4
                });

            var imageURL = 'assets/data/layers/nominalssites/0.svg'
            var cellGroupImage = cellGroup.append("svg:image")
                .attr('width', 45)
                .attr('height', 45.4)
