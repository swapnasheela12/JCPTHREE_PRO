...
@Component({
  selector: 'app-prediction-scheduling-details',
  templateUrl: 'prediction-scheduling-details.component.html',
  styleUrls: ['prediction-scheduling-details.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class PredictionSchedulingDetailsComponent implements OnInit {
  ...
  public selectedSiteSelection: FormGroup;
  public selectSiteStatusFormControl: FormGroup;
  public selectTechnologyFormControl: FormGroup;
  public selectPredictionTypeFormControl: FormGroup;
  public selectSiteTypeFormControl: FormGroup;
  public selectBandFormControl: FormGroup;
  ...
  selectedBoundaries: FormControl = new FormControl();
  selectedSiteStatusChk: FormControl = new FormControl();
  selectedSiteStatus: FormControl = new FormControl();
  selectedFrequency: FormControl = new FormControl();
  recurEveryControl: FormControl = new FormControl();
  selectedTechnology: FormControl = new FormControl();
  selectedPredictionType: FormControl = new FormControl();
  selectedSiteType: FormControl = new FormControl();
  selectedBand: FormControl = new FormControl();
  selectedSelectType: FormControl = new FormControl();
  ...
  selectTypeArr: string[] = ['Sunday', 'Monday', 'Tuesday','Wednesday','Thursday','Friday','Saturday'];
  showDiv: boolean = false;
  showDivSiteStatus = "Proposed Nominal";
  showDivFrequency = "Proposed Nominal";
  showDivTechnology = "4G (LTE)";
  showDivPredictionType = "RSRP";
  showDivSiteType = "Macro Sites";
  showDivBand = "3500 MHz";
  valueSearch = "";
  ...
  radioButtonTechnologyChanged(item) {
    console.log(item, "item");
    if (item.value == "Prediction Type") {
      this.showDivTechnology = "Prediction Type";
    }
    else if (item.value == "Upload Link Budget") {
      this.showDivTechnology = "Upload Link Budget";
    } else {
      this.showDivTechnology = "Define Cell Radius";
    }
  }

  radioButtonSiteStatusChanged(item) {
    console.log(item, "item");
    if (item.value == "Proposed Nominal") {
      this.showDivSiteStatus = "Proposed Nominal";
    }
    else if (item.value == "Upload Link Budget") {
      this.showDivSiteStatus = "Upload Link Budget";
    } else {
      this.showDivSiteStatus = "Define Cell Radius";
    }
  }

  radioButtonFrequencyChanged(item) {
    console.log(item, "item");
    this.typeGenerate = false;
    if (item.value == "Proposed Nominal") {
      this.showDivFrequency = "Proposed Nominal";
    }
    else if (item.value == "Upload Link Budget") {
      this.showDivFrequency = "Upload Link Budget";
    } else {
      this.showDivFrequency = "Define Cell Radius";
    }
  }

  radioButtonPredictionTypeChanged(item) {
    console.log(item, "item");
    // this.selectedPredictionType.setValue(item.value) ;
    // if (item.value == "Prediction Type") {
    //   this.showDivPredictionType = "Prediction Type";
    // }
    // else if (item.value == "Upload Link Budget") {
    //   this.showDivPredictionType = "Upload Link Budget";
    // } else {
    //   this.showDivPredictionType = "Define Cell Radius";
    // }
  }

  radioButtonSelectTypeChanged(item) {
    console.log(item, "item");
    // this.selectedPredictionType.setValue(item.value) ;
    // if (item.value == "Prediction Type") {
    //   this.showDivPredictionType = "Prediction Type";
  ...
  radioButtonSiteTypeChanged(item) {
    console.log(item, "item");
    if (item.value == "Macro Sites") {
      this.showDivSiteType = "Macro Sites";
    }
    else if (item.value == "Upload Link Budget") {
      this.showDivSiteType = "Upload Link Budget";
    } else {
      this.showDivSiteType = "Define Cell Radius";
    }
  }

  radioButtonBandChanged(item) {
    console.log(item, "item");
    if (item.value == "Prediction Type") {
      this.showDivBand = "Prediction Type";
    }
    else if (item.value == "Upload Link Budget") {
      this.showDivBand = "Upload Link Budget";
    } else {
      this.showDivBand = "Define Cell Radius";
    }
  }
  ...
  ngOnInit(): void {
    ...
    this.selectedBoundaries.setValue(this.listBoundaries[0]);
    // this.selectedSiteStatus = "Proposed Nominal";
    this.selectedSiteStatus.setValue(this.siteStatusArr[0]);

    this.selectSiteStatusFormControl = this._formBuilder.group({
    });

    this.selectedFrequency.setValue(this.frequencyArr[0]);

    this.selectedSelectType.setValue(this.selectTypeArr[0]);

    this.recurEveryControl.setValue('1');

    // this.selectFrequencyFormControl = this._formBuilder.group({
    // });

    // this.selectedTechnology = "4G (LTE)";
    this.selectedTechnology.setValue(this.technologyArr[0]);

    this.selectTechnologyFormControl = this._formBuilder.group({
    });

    // this.selectedPredictionType = "RSRP";
    this.selectedPredictionType.setValue([this.predictionTypeArr[0]]) ;

    this.selectPredictionTypeFormControl = this._formBuilder.group({
    });

    this.selectedSiteType.setValue(this.siteTypeArr[0]) ;

    this.selectSiteTypeFormControl = this._formBuilder.group({
    });

    this.selectedSiteSelection = this._formBuilder.group({
    });

    // this.selectedBand = "28 GHz";
    this.selectedBand.setValue(this.bandArr[0]);

    this.selectBandFormControl = this._formBuilder.group({
    });
