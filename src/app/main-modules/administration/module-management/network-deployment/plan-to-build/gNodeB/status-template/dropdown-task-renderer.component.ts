...
@Component({
  selector: 'dropdown-task-renderer',
  template: `
  
    <mat-form-field class="form-field-selected-query" style="width: 75%;">
    <mat-select disableOptionCentering [(value)]="task[0]" [disabled]="disable"
    style="font-family: lato Regular; font-size: 14px;">
    <mat-option class="sector-title" *ngFor="let t of task" [value]="t">
        {{t}}</mat-option>
</mat-select>
    </mat-form-field>
    `,
...
export class dropdownTaskRendererComponent implements ICellRendererAngularComp, OnInit {
  params: any;
  label: string;
